...
int main() // int argc, char** argv
{
    ...
    String proj_dir = "/home/bigfly/Documents/picture_mnist/";

    String test_data_path = proj_dir + "/test_data/";
    ...
    for (int i = 0; i < src_test.size(); i++)
    {
        ...
        oFile << src_test[i].substr(src_test[i].length() - 20, src_test[i].length()) << ","
              << "-" << sianum << endl;

        srcImage.release();
        oFile.close();
        cout << "the detect result of " << name << " is: -" << sianum << endl
             << "Infer done!" << endl
             << endl;
    }
